...
PORTRAIT_ACTION_PROMPT_TEMPLATES = {
    ...
    "rest": (
        "3秒循环动画。卡通人像自然站立并四处张望，保持放松站姿，身体重心自然落在双脚之间，"
        "可以有非常轻微的左右重心变化。上半身做轻微呼吸起伏，让角色看起来有生命感但不要夸张。"
        "头部偶尔缓慢转向左边和右边，像是在自然地四处张望，眼睛也跟随头部方向轻微移动。"
        "角色保持轻松温柔的表情，偶尔自然眨眼。头发、衣服边缘和裙摆随着呼吸和头部动作轻微摆动。"
        "整体动画要平滑、安静、可爱，适合长时间循环播放，不要有大幅度动作，不要像跳舞，"
        "动作节奏要慢一点，像角色正在安静陪伴用户，而不是主动表演。"
        "{style_phrase}根据首帧主体的性别保持一致：男性保持男性外貌和气质，女性保持女性外貌和气质。"
        f"{PORTRAIT_FRAME_SAFETY_RULE}{GREEN_SCREEN_RULE}"
        "站立张望时头部、头发、手臂、衣服、裙摆、腿和脚都不能接近或超出画面边缘。"
    ),
}
