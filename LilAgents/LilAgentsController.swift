...
class LilAgentsController {
    ...
    func start() {
        let pet = WalkerCharacter(videoLibrary: .current, name: "Pet")

        // Detect available providers, then set first-run defaults
        AgentProvider.detectAvailableProviders { [weak pet] in
        ...
        pet.accelStart = 0.15
        pet.fullSpeedStart = 0.45
        pet.decelStart = 2.55
        pet.walkStop = 3.0
        pet.walkAmountRange = 0.35...0.6
        pet.yOffset = -3
        pet.characterColor = NSColor(red: 0.4, green: 0.72, blue: 0.55, alpha: 1.0)
        pet.flipXOffset = 0
        pet.positionProgress = 0.5
        pet.pauseEndTime = CACurrentMediaTime() + Double.random(in: 0.5...2.0)
        pet.setup()
    ...
    private func triggerOnboarding() {
        guard let pet = characters.first else { return }
        pet.isOnboarding = true
        // Show "hi!" bubble after a short delay so the character is visible first
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
            pet.currentPhrase = "hi!"
            pet.showingCompletion = true
            pet.completionBubbleExpiry = CACurrentMediaTime() + 600 // stays until clicked
            pet.showBubble(text: "hi!", isCompletion: true)
