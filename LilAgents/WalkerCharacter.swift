...
class WalkerCharacter {
    ...
    let videoDuration: CFTimeInterval = 3.0
    var accelStart: CFTimeInterval = 0.15
    var fullSpeedStart: CFTimeInterval = 0.45
    var decelStart: CFTimeInterval = 2.55
    var walkStop: CFTimeInterval = 3.0
    ...
    init(videoLibrary: PetVideoLibrary = .current, name: String) {
    ...
    func setup() {
        guard let videoURL = videoLibrary.url(for: .idle) else {
            print("No pet video found for \(name)")
            return
        }
    ...
    func setAction(_ action: PetAction, shouldPlay: Bool = true, restart: Bool = false) {
        guard currentAction != action || restart else {
            if shouldPlay {
                queuePlayer.play()
            } else {
                queuePlayer.pause()
    ...
    func playTemporaryAction(_ action: PetAction, duration: CFTimeInterval) {
        actionBeforeTemporaryAction = currentAction ?? preferredActionForCurrentState()
        temporaryActionEndTime = CACurrentMediaTime() + duration
        setAction(action, shouldPlay: true, restart: true)
    }

    private func preferredActionForCurrentState() -> PetAction {
        if isIdleForPopover { return .rest }
        if isWalking { return .run }
        return .idle
    }

    private func restoreTemporaryActionIfNeeded(now: CFTimeInterval = CACurrentMediaTime()) {
        guard temporaryActionEndTime > 0, now >= temporaryActionEndTime else { return }

        temporaryActionEndTime = 0
        let nextAction = actionBeforeTemporaryAction ?? preferredActionForCurrentState()
        actionBeforeTemporaryAction = nil
        setAction(nextAction, shouldPlay: isManuallyVisible && environmentHiddenAt == nil)
    }
    ...
    func setManuallyVisible(_ visible: Bool) {
        ...
        if visible {
            if environmentHiddenAt == nil {
                window.orderFrontRegardless()
                setAction(preferredActionForCurrentState(), shouldPlay: true)
            }
        } else {
            queuePlayer.pause()
            window.orderOut(nil)
    ...
    private func openOnboardingPopover() {
        ...
        isWalking = false
        isPaused = true
        setAction(.rest, shouldPlay: true, restart: true)
    ...
    func startWalk() {
        ...
        updateFlip()
        setAction(.run, shouldPlay: true, restart: true)
    }
